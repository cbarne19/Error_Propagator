...
var = ["d/t"]
var2 = []
ky = ['a,b,c,aerr,berr,cerr = 10,5,0.5,0.75,0.76,4.21']    
a = []  
function = ['def F(a,b,c):\n    return a*b/c'] 
vers = [] 
...
varis2 = [] 
def submit():   
    var.append(name_entry_Voltage.get())
    var2.append(name_entry.get())
    expression = passw_entry.get()
    ...
    QWERTY = eval(form)
    #print(QWERTY)
    _C = dydxs()# 3 #errorfinder(form,val)
    name_var_fin_t.set(str(round(QWERTY,7))+'±'+str(_C))
            
    

name_label_Voltage = tk.Label(root, text = 'Input formula', font=('calibre',10, 'bold'))
name_entry_Voltage = tk.Entry(root,textvariable = Voltage_var,width = 40, font=('calibre',10,'normal'))
...
passw_label = tk.Label(root, text = 'Values', font = ('calibre',10,'bold'))
passw_entry=tk.Entry(root, textvariable = passw_var, width = 40,font = ('calibre',10,'normal'))
...
name_label_Voltage.grid(row=2,column=0)
name_entry_Voltage.grid(row=2,column=1)
